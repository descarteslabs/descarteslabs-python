...
class Runcible(Service):
    ...
    def keys(self, const_id=None, shape=None, geom=None, start_time=None, end_time=None, params=None, limit=100, offset=0):

        """Get only the keys from a search
        ...
        def f(x):
            ...
            return [feature['id'] for feature in r.json()]

        if const_id is None:
            const_id = [None]

        result = list(chain(*map(f, const_id)))
